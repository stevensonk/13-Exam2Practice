...
class Box(object):
    ...
    def append_string(self, additional_contents):
        ...
        for k in range(len(additional_contents)):
            if len(self.contents) == self.volume:
                for n in range(k, len(additional_contents)):
                    leftovers = leftovers + additional_contents[n]
                return leftovers
            elif k == len(additional_contents) - 1:
    ...
    def shrink(self, new_volume):
        ...
        if len(self.contents) > new_volume:
            for n in range(new_volume):
                new_content = new_content + self.contents[n]
            self.contents = ''
    ...
    def double_then_shrink(self, new_volume):
        ...
        doubled = self.double()
        shrunk = self.shrink(new_volume)
        total = str(doubled) + str(shrunk)
        return len(total)
